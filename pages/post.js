...
const Post = ({ post, role }) => {
  ...
  const lang = (isAuthor || isEditor)
    ? useContext(LangContext)
    : (post.exclusive || useContext(LangContext));
  ...
  const choose = (img) => {
    fetch('/post/edit/changeCover', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ img, post: post._id }),
    });
    chooseCover(img);
  ...
  const publish = async (exclusive, isPublic = true) => {
    ...
    const data = await fetch('/post/publish', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ post: post._id, exclusive, public: isPublic }),
    }).then(res => res.json());
    togglePublish();
    if (data.url) {
      Router.push({
  ...
  const save = async (name, value) => fetch('/post/edit/text', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      lang, post: post._id, [name]: value, name,
    }),
  }).then(res => res.json());
